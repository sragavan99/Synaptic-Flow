...
class RSFGrad(Pruner):
    def __init__(self, masked_parameters, clone=True):
        super(RSFGrad, self).__init__(masked_parameters, clone=clone)
    ...
    def score(self, model, loss, dataloader, device):
        ...
        for _, p in self.masked_parameters:
            self.scores[id(p)] = torch.clone(p.grad).detach().abs_()
            p.grad.data.zero_()
...
# implements a rewind for generic pruner
